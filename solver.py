# Cloned from https://github.com/hanqiu92/itapas
...
class ITAPAS(object):
    ...
    def __init__(
        self, network, link_param, link_background, link_cost_func, ori_list=None
    ):
        """
        # input
        ...
        for i in range(len(edge_list)):
            curr_edge_idx = network.edge_index[
                network.edge(edge_list[i, 0], edge_list[i, 1])
            ]
    ...
    def MFS(
        self, flow_map, local_flow_map, cost_map, cost_de_map, prev_map, r, e0, epsilon
    ):
        ...
        v_list, e_list = gt.shortest_path(
            network,
            source=network.vertex(r),
            target=network.vertex(e0[1]),
            pred_map=prev_map,
        )
        if e0[0] == r:
            return (
                tuple([edge_index.get(tuple(e)) for e in e_list]),
                tuple([edge_index.get(e0)]),
            )
        ...
        while True:
            ...
            while inner_flag:
                ...
                if status_map[m] == -1:
                    # step 3: return pas
                    _, e1 = gt.shortest_path(
                        network,
                        source=network.vertex(m),
                        target=network.vertex(j),
                        pred_map=prev_map,
                    )
                    _, e2 = gt.shortest_path(
                        network,
                        source=network.vertex(m),
                        target=network.vertex(j),
                        pred_map=prev_new_map,
                    )
                    return (
                        tuple([edge_index.get(tuple(e)) for e in e1]),
                    ...
                    _, cost_map.a[e_list], cost_de_map.a[e_list] = link_cost_func(
                        flow_map.a[e_list] + link_background.a[e_list],
    ...
    def shift(
        ...
        if f1 < epsilon and f2 < epsilon:
            ...
            f1 < epsilon or f2 < epsilon or np.abs(t2 - t1) < mu * (t2 + t1)
        ) and not init_flag:
            return None
        else:
            ...
            ind_e = ind_e1 + ind_e2
            _, link_cost.a[ind_e], link_cost_de.a[ind_e] = cost_func(
                link_flow.a[ind_e] + link_background.a[ind_e],
    ...
    def get_duality_gap(self, link_flow, link_cost, demand):
        ...
        for ori in self.ori_index.keys():
            d_map = gt.shortest_distance(
                network, source=network.vertex(ori), weights=link_cost, max_dist=100000
            )
    ...
    def assign(
        ...
        if not init_flow or not init_flow_node:
            ...
            subnet = {}
            for ori in ori_index.keys():
                _, subnet[ori] = gt.shortest_distance(
                    network,
                    source=network.vertex(ori),
                    weights=link_cost,
                    max_dist=100000,
                    pred_map=True,
                )
            for key, value in demand.items():
                _, e_list = gt.shortest_path(
                    network,
                    source=network.vertex(key[0]),
                    target=network.vertex(key[1]),
        ...
        _, link_cost.a, link_cost_de.a = cost_func(
            link_flow.a + link_background.a, [p.a for p in self.link_param]
        )
        dg = self.get_duality_gap(link_flow, link_cost, demand)
        if out_flag:
            print(
                "ITAPAS: Initial duality gap = {:.0f}.  Elapsed time = {:.2f}.".format(
                    dg, time.time() - t
                )
            )
        if dg < dg_limit:
        ...
        while flag:
            ...
            for ori in ori_index.keys():
                ...
                d_map, prev_map = gt.shortest_distance(
                    network,
                    source=network.vertex(ori),
                    weights=link_cost,
                    max_dist=10000,
                ...
                comp_slack = float(
                    np.sum(local_link_flow.a * (rc_start - rc_end + link_cost.a))
                )
                if comp_slack > 1e-8:
                    ind_list = list(
                        np.logical_and(
                            local_link_flow.a > epsilon * comp_slack,
                ...
                if pas_set:
                    ...
                    if len(pas_list) > 100:
                        pas_list_select = [
                            ...
                            for idx in np.random.choice(
                                len(pas_list), 100, replace=False
                            )
                        ]
                    else:
                    ...
                    for pas in pas_list_select:
                        self.shift(
                            link_flow,
                            link_flow_node[pas_set.get(pas)],
                            link_cost,
                            link_cost_de,
                            pas,
                            epsilon,
            ...
            if out_flag:
                print(
                    "ITAPAS: Iteration {} finished. PAS size = {}. Duality gap = {:.0f}. Elapsed time = {:.2f}.".format(
                        curr_iter, len(pas_set), dg, time.time() - t
                    )
                )
            curr_iter += 1
            if (
...
def bpr_function(x, params):
    ...
    return (
        ...
        params[3]
        * (params[0] * params[1] * x ** (params[1] - 1) / params[2] ** params[1]),
    )
